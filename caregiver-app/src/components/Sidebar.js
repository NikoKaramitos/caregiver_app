...
const Sidebar = () => {
  ...
  useEffect(() => {
    fetch("http://localhost/getCareGivers.php") // Replace with your server URL
      .then((response) => {
        if (!response.ok) {
  ...
  useEffect(() => {
    ...
    const filtered = caregivers.filter((caregiver) => {
      ...
      const matchesRating =
        filters.rating === "" || caregiver.reviews >= parseInt(filters.rating);
      const matchesCity =
        filters.city === "" ||
        caregiver.city.toLowerCase().includes(filters.city.toLowerCase());
      return matchesRate && matchesRating && matchesCity;
    });
    setFilteredCaregivers(filtered);
  }, [filters]); // Re-run the filter whenever `filters` is updated
      ...
      <div style={styles.container2}>
        ...
        {selectedCaregiver && (
          <div style={styles.details}>
            ...
            />
            <h2 style={styles.detailsName}>{selectedCaregiver.name}</h2>
            ...
            <p style={styles.detailsInfo}>
              <strong>Rating:</strong> {renderStars(selectedCaregiver.reviews)}{" "}
            ...
            <button
              ...
              onClick={() => handleContact(selectedCaregiver.name)}
            >
              Contact
            </button>
          </div>
